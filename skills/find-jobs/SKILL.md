...
4. `get_job` or `get_jobs` for the full text of the ones that survived. A `https://resumebooster.work/jobs?job=<id>` link's `id` is the argument `get_job`, `fetch`, `check_apply_support` and `request_application` take.
...
Rules: never invent a posting, salary or employer fact the tools did not return; show pay only when the card states it; say "unknown" when the tool does. Tools that need a key or a pass: name the gate (https://resumebooster.work/data-api for a key, https://resumebooster.work/agents/pass for a pass), never a price.
